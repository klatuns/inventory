...
// saving the receipt
router.post("/sale/receipt", function(req, res) {
    var products = req.body;
    let myArray = Object.values(products);
    let myObj = Object.keys(myArray[0]);

    myObj = JSON.parse(myObj);

    console.log(myObj);

    res.redirect("/sale/receipt");
});
...
router.put("/product/:id", (req, res) => {
    Product.findById(req.params.id, (err, foundId) => {
        if (err) {
            ...
            // console.log(obj);
            Product.findByIdAndUpdate(
                req.params.id, { quantity: newQuantity },
                (err, foundQuantity) => {
                    if (err) {
                        console.log(err);
                    } else {
                        // console.log(
                        //     "Name " + foundQuantity.name,
                        //     "Price " + foundQuantity.price * obj.quantity,
...
//                 console.log(err);
//             } else {
//                 var name = foundQuantity.name;
//                 var price = foundQuantity.price * obj.quantity;
//                 var quantity = obj.quantity;
//                 var code = foundQuantity.code;
//                 var newHistory = {
//                     name: name,
//                     price: price,
//                     quantity: quantity,
